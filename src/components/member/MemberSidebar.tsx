...
export default function MemberSidebar({ isOpen = false, onClose }: SidebarProps) {
  ...
  const memberLinks = [
    { href: "/member-dashboard", label: t("dashboard"), icon: "◉" },
    ...(role === "INCHARGE" 
      ? [{ href: "/group-reports", label: "Group Reports", icon: "📊" }] 
      : []),
    { href: "/mark-attendance", label: t("mark_attendance"), icon: "📍" },
    { href: "/my-records", label: t("my_records"), icon: "📋" },
    { href: "/activity", label: t("activity"), icon: "⚡" },
    { href: "/profile", label: t("profile"), icon: "👤" },
    { href: "/change-password", label: t("change_password"), icon: "🔐" },
  ];
        ...
        <div className="flex items-center justify-between border-b border-white/10 p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">{t("member_portal")}</p>
            </div>
          </div>
          {onClose && (
            <button
